...
CONFIG = {
    ...
    "LANGUAGE": os.getenv("TRENDSENSE_LANG", "tr"),
    "SOURCE": "trendsense",
}
...
BODY_TEMPLATES_LOCAL_EN = [
    """{keyword} {location} — Latest updates and comprehensive information as of {date}. Stay informed about the latest developments regarding {keyword} in the {location} area.

{category_intent_lines}

We closely monitor developments regarding {keyword} in {location} and surrounding areas. For the most accurate and current information on this important topic affecting local residents, visit our page.

SEO target: Optimized content for {location} {keyword} search rankings.""",

    """{keyword} {location} — {date} current status, expert analysis and regional impact assessment.

{category_intent_lines}

The topic of {keyword} is of great importance to {location} residents. We continuously add all official statements and developments to our page in real-time."""
]

BODY_TEMPLATES_LOCAL_RU = [
    """{keyword} {location} — Актуальная информация и последние новости на {date}. Будьте в курсе последних событий о {keyword} в регионе {location}.

{category_intent_lines}

Мы внимательно следим за развитием событий по теме {keyword} в {location} и окрестностях. Самая точная и актуальная информация на нашей странице.""",

    """{keyword} {location} — {date} текущая ситуация, экспертный анализ.

{category_intent_lines}

Тема {keyword} имеет большое значение для жителей {location}. Мы добавляем все официальные заявления и события в реальном времени."""
]

BODY_TEMPLATES_LOCAL_FR = [
    """{keyword} {location} — Dernières informations et mise à jour complète au {date}. Restez informé des derniers développements concernant {keyword} dans la région de {location}.

{category_intent_lines}

Nous suivons de près les développements concernant {keyword} dans {location} et ses environs.""",

    """{keyword} {location} — {date} situation actuelle, analyse d'experts.

{category_intent_lines}

Le sujet de {keyword} est d'une grande importance pour les habitants de {location}. Nous ajoutons toutes les déclarations officielles en temps réel."""
]
...
class ContentMatrix:
    ...
    def __init__(self, classifier: EntityClassifier, lang: str = "tr"):
    ...
    def _get_templates(self, rule: str, lang: str) -> List[str]:
        if rule == "A":
            if lang == "tr":
                return BODY_TEMPLATES_LOCAL
            elif lang == "en":
                return BODY_TEMPLATES_LOCAL_EN
            elif lang == "ru":
                return BODY_TEMPLATES_LOCAL_RU
            elif lang == "fr":
                return BODY_TEMPLATES_LOCAL_FR
            else:
                return BODY_TEMPLATES_LOCAL
        elif rule == "B":
            return BODY_TEMPLATES_GLOBAL
        else:
            return BODY_TEMPLATES_NATIONAL
    ...
    def _safe_body(self, keyword: str, location: str, intent_lines: str,
                   templates: List[str], lang: str = "tr") -> str:
        ...
        if not body or len(body.strip()) < 50:
            ...
            if lang == "tr":
                body = (
                    ...
                    f"en son gelişmeleri bulabilirsiniz.\n\n{intent_lines}"
                )
            elif lang == "en":
                body = (
                    f"{keyword}{loc_part} — {today_str()} comprehensive information. "
                    f"Find the latest developments about {keyword}{loc_part} on this page.\n\n{intent_lines}"
                )
            elif lang == "ru":
                body = (
                    f"{keyword}{loc_part} — {today_str()} подробная информация. "
                    f"Последние новости о {keyword}{loc_part} на этой странице.\n\n{intent_lines}"
                )
            elif lang == "fr":
                body = (
                    f"{keyword}{loc_part} — {today_str()} informations complètes. "
                    f"Dernières actualités sur {keyword}{loc_part} sur cette page.\n\n{intent_lines}"
                )
        return body
    ...
    def generate_local_matrix(self, classification: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        lang = self.lang
        templates = self._get_templates("A", lang)
        lang_meta = LANG_META.get(lang, LANG_META["tr"])
        nodes: List[Dict[str, Any]] = []

        for location in ALL_LOCATIONS:
            loc_slug = tr_slug(f"{lang_meta['suffix']}{location}")
            intent_lines = self._build_intent_lines(intent_kw, keyword, location)

            if lang == "tr":
                title = f"{keyword} {location} — {today_str()} Güncel Durum"
                meta_desc = f"{keyword} {location} için en güncel bilgiler. {today_str()} itibarıyla {location} bölgesinde {keyword} ile ilgili tüm detaylar."
            elif lang == "en":
                title = f"{keyword} {location} — {today_str()} Current Status"
                meta_desc = f"Latest information about {keyword} in {location}. All details about {keyword} as of {today_str()}."
            elif lang == "ru":
                title = f"{keyword} {location} — {today_str()} Tекущая ситуация"
                meta_desc = f"Актуальная информация о {keyword} в {location}. Все подробности о {keyword} по состоянию на {today_str()}."
            elif lang == "fr":
                title = f"{keyword} {location} — {today_str()} Situation actuelle"
                meta_desc = f"Informations à jour sur {keyword} à {location}. Tous les détails sur {keyword} au {today_str()}."

            slug = f"{lang_meta['suffix']}{generate_deterministic_slug(keyword, loc_slug)}"
            body_content = self._safe_body(keyword, location, intent_lines, templates, lang)
    ...
    def generate_global_article(self, classification: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        cat_slug = tr_slug(cat_label)
        lang = self.lang
        templates = self._get_templates("B", lang)
        lang_meta = LANG_META.get(lang, LANG_META["tr"])
        intent_lines = self._build_intent_lines(intent_kw, keyword)

        if lang == "tr":
            title = f"{keyword} — {today_str()} Kapsamlı Rehber ve Güncel Bilgiler"
            meta_desc = f"{keyword} hakkında en kapsamlı Türkçe kaynak. {today_str()} itibarıyla güncellenen bu sayfada {keyword} ile ilgili merak ettiğiniz her şey."
        elif lang == "en":
            title = f"{keyword} — {today_str()} Comprehensive Guide"
            meta_desc = f"The most comprehensive guide on {keyword}. Updated as of {today_str()}."
        elif lang == "ru":
            title = f"{keyword} — {today_str()} Подробное руководство"
            meta_desc = f"Самое подробное руководство по {keyword}. Обновлено {today_str()}."
        elif lang == "fr":
            title = f"{keyword} — {today_str()} Guide complet"
            meta_desc = f"Le guide le plus complet sur {keyword}. Mis à jour le {today_str()}."

        slug = f"{lang_meta['suffix']}{generate_deterministic_slug(keyword, 'global')}"
        body_content = self._safe_body(keyword, "", intent_lines, templates, lang)
    ...
    def generate_national_article(self, classification: Dict[str, Any]) -> List[Dict[str, Any]]:
        keyword = classification["keyword"]
        lang = self.lang
        templates = self._get_templates("C", lang)
        lang_meta = LANG_META.get(lang, LANG_META["tr"])
        ...
        if lang == "tr":
            ...
            meta_desc = f"{keyword} ile ilgili son dakika gelişmeleri. {today_str()} itibarıyla Türkiye gündemindeki {keyword} konusunda tüm detaylar."
            loc_label = "Türkiye"
        elif lang == "en":
            title = f"{keyword} — {today_str()} Breaking News"
            meta_desc = f"Latest breaking news about {keyword} in Turkey. Updated {today_str()}."
            loc_label = "Turkey"
        elif lang == "ru":
            title = f"{keyword} — {today_str()} Последние новости"
            meta_desc = f"Последние новости о {keyword} в Турции. Обновлено {today_str()}."
            loc_label = "Турция"
        elif lang == "fr":
            title = f"{keyword} — {today_str()} Dernières nouvelles"
            meta_desc = f"Dernières nouvelles sur {keyword} en Turquie. Mis à jour le {today_str()}."
            loc_label = "Turquie"

        slug = f"{lang_meta['suffix']}{generate_deterministic_slug(keyword, 'national')}"
        body_content = self._safe_body(keyword, loc_label, intent_lines, templates, lang)
...
class TrendSenseBot:
    ...
    def __init__(self, dry_run: bool = False, quick: bool = False, resume: bool = False, lang: str = "tr"):
        self.dry_run = dry_run
        self.quick = quick
        self.resume_mode = resume
        self.lang = lang if lang in LANG_META else "tr"
        CONFIG["LANGUAGE"] = self.lang
        self.checkpoint = Checkpoint()
        self.classifier = EntityClassifier()
        self.matrix = ContentMatrix(self.classifier, self.lang)
        self.api = ApiClient()
...
def main():
    ...
    parser.add_argument(
        ...
        default="tr",
        choices=["tr", "en", "ru", "fr"],
        help="Content language (tr/en/ru/fr)",
    )
